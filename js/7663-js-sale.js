const CONTRACT_ADDR = "0x3aADade9954a93edf32C9afE5A0817c6ee0DFEcF";
  ...
  if (typeof window.ethereum !== 'undefined') {
      ...
      if (_mintAmount > 0) {
        ...
        try {
          ...
          const cost = await contract.methods.cost().call()
          const value = (cost * _mintAmount)
          const gas = Math.round( await contract.methods.whitelistMint(_mintAmount).estimateGas({value: value.toString(), from: accounts[0]}) * 1.1 )
          result = await contract.methods.whitelistMint(_mintAmount).send({value: value.toString(), from: accounts[0], gas: gas})
